...
async function handleCommand(sock: WASocket, msg: WAMessage): Promise<void> {
  ...
  const attachmentObj = await extractAttachment(msg);
...
async function collectLink(sock: WASocket, msg: WAMessage, groupJid: string): Promise<void> {
  ...
  for (const url of extracted) {
    const exists = db.select().from(model.links).where(eq(model.links.url, url)).limit(1).get();
    if (exists) continue;

    db.insert(model.links).values({ url, senderName, sourceGroupName }).onConflictDoNothing().run();
    added++;
  }
...
async function maybeSendLinkReport(sock: WASocket): Promise<void> {
  const result = db.select({ value: count() }).from(model.links).where(eq(model.links.isSent, false)).get();
  if (!result || result.value < 5) return;

  const unsent = db.select().from(model.links).where(eq(model.links.isSent, false)).all();
  ...
  try {
    ...
    db.update(model.links).set({ isSent: true }).where(eq(model.links.isSent, false)).run();
