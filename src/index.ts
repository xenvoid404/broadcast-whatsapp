...
async function main(retries: number = 0): Promise<void> {
  try {
    ...
    if (env.app.method === 'pairing' && !sock.authState.creds.registered) {
      setTimeout(async () => {
        try {
          logger.info({ number: env.app.number }, 'Meminta kode pairing...');
          const code = await sock.requestPairingCode(env.app.number);
